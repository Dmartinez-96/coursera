  ...
  for (unsigned x = 0; x < image.width(); x++) {
    for (unsigned y = 0; y < image.height(); y++) {
      if (((int)x == centerX && (int)y == centerY)) {
        ...
        HSLAPixel & pixel = image.getPixel(x, y);
        pixel.l = max(pixel.l - (sqrt(((x - centerX) * (x - centerX))
                                      + ((y - centerY) * (y - centerY)))
                                 * 0.005 * pixel.l), 0.2 * pixel.l);
      }
    }
  }
...
PNG illinify(PNG image) {
  for (unsigned x = 0; x < image.width(); x++) {
    for (unsigned y = 0; y < image.height(); y++) {
      ...
      double distToO = fmin(fabs(pixel.h - 11.0), 360.0 - fabs(pixel.h - 11.0));
      double distToB;

      pixel.h = (fmin(fabs(pixel.h - 11.0), 360.0 - fabs(pixel.h - 11.0)) < fmin(fabs(pixel.h - 216.0), 360.0 - fabs(pixel.h - 216.0))) ? 11.0 : 216.0;
    }
  }
  return image;
}
